...
# 按实时市值分配资金
def allocate_funds_by_realtime_market_cap(tickers, start_date, end_date, initial_cash):
    market_caps = {}
    ...
    total_market_cap = sum(market_caps.values())  # 计算所有公司的总市值
    allocation = {}
    
    for ticker in tickers:
        if market_caps[ticker] > 0:
            allocation[ticker] = (market_caps[ticker] / total_market_cap) * initial_cash  # 按市值比例分配
        else:
            allocation[ticker] = 0  # 没有市值数据的公司不分配资金
    return allocation
...
def test_different_moving_averages():
    ...
    # 按实时市值分配初始资金
    allocation = allocate_funds_by_realtime_market_cap(world_1000_tickers, start_date, end_date, initial_cash)
    ...
    df_results.to_csv('moving_average_test_results_realtime_market_cap.csv', index=False)
    print("\nResults saved to 'moving_average_test_results_realtime_market_cap.csv'.")
