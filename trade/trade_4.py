...
def backtest_strategy(moving_average_days):
    ...
    for ticker in world_1000_tickers:
        ...
        for index, row in stock_data.iterrows():
            ...
            if current_price < moving_avg * 0.93:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.92:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.91:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.90:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.89:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.88:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.87:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.86:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.85:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.84:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.83:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.82:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.81:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.80:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.78:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.77:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.76:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.75:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
            ...
            if current_price < moving_avg * 0.74:  # 没有持仓时买入
                shares_to_buy = allocation_per_stock // current_price
                if shares_to_buy > 0:
                    portfolio[ticker] += shares_to_buy
                    cash -= shares_to_buy * current_price
...
def test_different_moving_averages():
    results = []
    for days in range(200, 300):  # 测试从100天到109天的均线
    ...
    # 将结果写入 CSV 文件
    df_results.to_csv('moving_average_test_results_4.csv', index=False)
