...
for i in range(1, len(data)):
    if data[i] > data[i - 1]:  # Price went up
        ...
        if shares > 0:  # Check if there are shares to sell
            ...
            shares = 0
            print(f"Day {i}: Price up to {data[i]:.2f}. Sold all shares. Cash: ${cash:.2f}, Shares: {shares:.2f}")
    elif data[i] <= data[i - 1]:  # Price went down
        # Buy as many shares as possible
        if cash > 0:  # Check if there is cash to buy shares
            shares = cash / data[i]  # Convert all cash to shares
            cash = 0
            print(f"Day {i}: Price down to {data[i]:.2f}. Bought shares. Cash: ${cash:.2f}, Shares: {shares:.2f}")


# Final portfolio value
final_value = cash + shares * 228.26
