...
def main():
    ticker_list = world_1000_tickers  # Limit to 10 tickers for testing
    all_frequencies = analyze_ticker_data(ticker_list)
    
    # Convert to DataFrame
    result_df = pd.DataFrame(all_frequencies).fillna(0).astype(int)

    # Sort the DataFrame by its index (percentages) in ascending order
    result_df = result_df.sort_index()

    # Save the sorted DataFrame to CSV
    result_df.to_csv("percent_frequency_analysis_sorted.csv")
    print("Saved sorted results to 'percent_frequency_analysis_sorted.csv'")
