...
# Function to fetch and calculate average price
def fetch_average_price(ticker, max_days):
    """Fetch historical data and calculate the average closing price over the last `max_days`."""
    ...
    try:
        ...
        valid_prices = bars.df['close'][bars.df['close'] > 0]  # Exclude invalid prices (e.g., non-positive values)
        total_valid_prices = len(valid_prices)

        if total_valid_prices == 0:
            print(f"No valid closing prices found for {ticker}.")
            return None, None, None

        if total_valid_prices > max_days:
            recent_prices = valid_prices[-max_days:]  # Use the most recent `max_days` prices
        else:
            recent_prices = valid_prices  # Use all available valid prices

        avg_price = recent_prices.mean()  # Calculate the average price
        current_price = recent_prices.iloc[-1]  # Use the most recent valid price
        percentage = (current_price / avg_price) * 100  # Calculate the percentage
...
results = []
for idx, ticker in enumerate(world_1000_tickers):
    print(f"Processing {idx + 1}/{len(world_1000_tickers)}: {ticker}")
    ...
    if current_price and avg_price:
        results.append({
            ...
            'Price for +10%': avg_price * ((percentage + 10) / 100)  # Calculate price for current percentage + 10%
        })
...
if sorted_results:
    ...
    print(f"The ticker with the lowest percentage is {lowest_percentage_ticker['Ticker']} "
          f"at {lowest_percentage_ticker['Percentage']:.2f}% with a limit price of "
          f"{lowest_percentage_ticker['Price for +10%']:.2f}.")
